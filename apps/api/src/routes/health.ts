...
healthRoutes.get("/health", async (c) => {
  ...
  const ponder = env.success
    ? await createPonderReadSource({ graphqlUrl: env.data.PONDER_GRAPHQL_URL }).health()
    : indexerHealth();
  ...
  return c.json({
    ...
    dependencies: {
      api: "ok",
      ponder,
      contractRpc: env.success && (env.data.CONTRACT_RPC_URL || env.data.PONDER_RPC_URL) ? "configured" : "missing",
    ...
    persistence: env.success
      ? {
          mode: env.data.FX_BENTO_DATABASE_URL || env.data.PONDER_SQL_URL ? "postgres" : "sqlite",
